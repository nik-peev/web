var movingDivs = (function () {
    ...
    var divsMovingInEllipse = document.getElementsByClassName("ellipse");
    var divsMovingInRectangle = document.getElementsByClassName("rectangle");
    ...
    function add(orbitType) {
        ...
        setElementStyle(movingDiv, DIV_WIDTH, DIV_HEIGHT, MIN_TOP, MIN_LEFT, MAX_TOP, MAX_LEFT, BORDER_RADIUS, BORDER_WIDTH);

        if (orbitType === "ellipse") {
            movingDiv.classList.add("ellipse");
            ellipseDegreePositions.push(0);
            ellipseCenters.push({
                top: parseInt(movingDiv.style.top),
                left: parseInt(movingDiv.style.left) - MAJOR_SEMIAXIS
            });
        } else {
            movingDiv.classList.add("rectangle");
